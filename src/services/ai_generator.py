...
{{
    ...
    try:
        ...
        stats = {
            "base_health": (STAT_RANGES['CLASS_HEALTH'][0], STAT_RANGES['CLASS_HEALTH'][1]),
            "base_attack": (STAT_RANGES['CLASS_ATTACK'][0], STAT_RANGES['CLASS_ATTACK'][1]),
            "base_defense": (STAT_RANGES['CLASS_DEFENSE'][0], STAT_RANGES['CLASS_DEFENSE'][1]),
        ...
        for skill in data["skills"]:
            skill_data = {
                "name": str(skill["name"]),
                "damage": max(STAT_RANGES['SKILL_DAMAGE'][0], min(STAT_RANGES['SKILL_DAMAGE'][1], int(skill["damage"]))),
                "mana_cost": max(STAT_RANGES['SKILL_MANA_COST'][0], min(STAT_RANGES['SKILL_MANA_COST'][1], int(skill["mana_cost"]))),
