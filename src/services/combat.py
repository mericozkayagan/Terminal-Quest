...
def combat(
    ...
    combat_log = []
    boss_service = BossService() if any(isinstance(enemy, Boss) for enemy in enemies) else None
    ...
    while player_queue and enemy_queue:
        ...
        for player in player_queue:
            ...
            if choice == "1":  # Attack
                ...
                combat_log.insert(
                    0, f"{sym['ATTACK']} {player.name} strikes {target.name} for {player_damage} damage!"
                )
                ...
                try:
                    ...
                    if 0 <= skill_choice < len(player.skills):
                        ...
                        if player.mana >= skill.mana_cost:
                            ...
                            skill_damage = calculate_damage(player, target, skill.damage + random.randint(-3, 3))
                            ...
                            combat_log.insert(
                                0, f"{sym['MANA']} {player.name} consumed {skill.mana_cost} mana"
                            )
                        else:
                            combat_log.insert(0, f"{sym['MANA']} {player.name} does not have enough mana!")
                    else:
                    ...
                    if 0 <= item_choice < len(usable_items):
                        ...
                        if item.use(player):
                            ...
                            if item.name == "Health Potion":
                                combat_log.insert(
                                    0, f"{sym['HEALTH']} {player.name} restored {item.value} health!"
                                )
                            if item.name == "Mana Potion":
                                combat_log.insert(
                                    0, f"{sym['MANA']} {player.name} restored {item.value} mana!"
                                )
                        else:
                            combat_log.insert(0, f"{player.name} couldn't use that item right now!")
                    else:
